...
def check_input(user_input, valid_inputs):
    ...
    while user_input not in valid_inputs and len(user_input) <= 0:
        try:
...
def set_up_char(character_name):
   '''fetches the values and decks, etc. to set up player or opponent'''
   if character_name == ayato.char_name:
        return (ayato.hand, ayato.deck, ayato.card_effect_dict, ayato.char_values, ayato.misc_effects)
   elif character_name == furina.char_name:
        
        return (furina.hand, furina.deck, furina.card_effect_dict, furina.char_values, furina.misc_effects)

#helps assign the right character to the right target value/"target"
target_values_dict = { ayato.char_name : ayato.target_values,
                     furina.char_name : furina.target_values
}
...
while True:
    ...
    if (answer == "1"):
        ...
        answer = check_input(user_input, "0123456789")
        if answer == "0":
            continue
        player_character = roster[int(answer)-1]
        hub.p_hand, hub.p_deck, hub.p_card_effect_dict, hub.p_values, hub.p_misc_effects = set_up_char(player_character)
        print_space()
        ...
        print_space()
        #hard code for now accepted inputs
        answer = check_input(user_input, "123456")
        opponent_character = roster[int(answer)-1]
        hub.op_hand, hub.op_deck, hub.op_card_effect_dict, hub.op_values, hub.op_misc_effects = set_up_char(opponent_character)
        clear()
        target_dict_2 = {"p" : hub.op_values, "op": hub.p_values}
        print('here')
        print(hub.op_values)
        print(target_dict_2["p"])
        target_values_dict[player_character] = target_dict_2["p"]
        target_values_dict[opponent_character] = target_dict_2["op"]
        print(ayato.target_values == target_values_dict[player_character])

        '''
        for char_target_pair in ((player_character, "p"), (opponent_character), "op"):
        #this is a bandaid fix for the whole assigning target value
        
            if char_target_pair[0] == ayato.char_name:
                ayato.target_values = target_dict_2[char_target_pair[1]]
                print(ayato.target_values)
                print(target_dict_2["p"])
                print(char_target_pair[1])
            if char_target_pair[0] == furina.char_name:
                furina.target_values = target_dict_2[char_target_pair[1]]
        ayato.target_values = hub.op_values
        '''
        break
...
p_misc_effects = hub.p_misc_effects
op_misc_effects = hub.op_misc_effects
#modifiers, passive stacks, etc.
p_values = hub.p_values
op_values = hub.op_values
#dictionary of all cards in player's character's deck
p_card_effect_dict = hub.p_card_effect_dict
p_hand = hub.p_hand
p_deck = hub.p_deck
op_card_effect_dict = hub.op_card_effect_dict
op_hand = hub.op_hand
op_deck = hub.op_deck
...
def display_board_state():
    ...
    print_space()
    print(f"{opponent_character}(Opponent): [{op_values["hp"] * " ♥ "}]({op_values["hp"]}♥)")
    print("\n" * 2)
    print(op_board_text)
    print("\n" * 3)
    ...
    print(p_board_text)
    print("\n" * 2)
    print(f"{player_character}(You): [{p_values["hp"] * " ♥ "}({p_values["hp"]}♥)]")
    print(f"\nYour hand: ")
...
def play_card_input():
    ...
    while make_choice:
        ...
        if answer == "2":
            clear()
            print_char_info(roster.index(player_character) - 1)
...
def determine_winner():
    ...
    op_pow = op_chosen_card.power + op_values["temp_pow_mod"] + op_values["perm_pow_mod"]
    p_speed = speed_rankings.index(p_chosen_card.speed)
    p_pow = p_chosen_card.power + p_values["temp_pow_mod"] + p_values["perm_pow_mod"]
...
#draw hand for opponent
    ...
    p_board_text = p_chosen_card.display_name + "(+" + str(p_values["temp_pow_mod"] + p_values["perm_pow_mod"]) + ")" 
    op_board_text = op_chosen_card.display_name + "(+" + str(op_values["temp_pow_mod"] + op_values["perm_pow_mod"]) + ")"
    display_board_state()
    input("Enter any key to continue")
    clear()
    ...
    p_board_text = p_chosen_card.lose_text
    op_board_text = op_chosen_card.lose_text

    if(p_win_bool):
        p_board_text = p_chosen_card.win_text
    if(op_win_bool):
        op_board_text = op_chosen_card.win_text
...
if p_values["hp"] > 0 and op_values["hp"] <= 0:
    ...
    print_space()
    print(f"{opponent_character} wins!") 
    print_space()
else:
